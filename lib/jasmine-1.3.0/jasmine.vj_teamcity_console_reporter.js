            ...
            var eachSpecFn = function(spec){
                if (spec.description) {
                    ...
                    var specResultFn = function(result){
                        if (!result.passed_) {
                            outerThis.log("##teamcity[testFailed name='" + outerThis.escapeTeamcityString(spec.description) + "' message='|[FAILED|]' details='" + outerThis.escapeTeamcityString(result.trace.message) + "']");
                        }
                    };
